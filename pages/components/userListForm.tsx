//userListForm.tsx pm 2시부터 작업하던 것들.. 
...
interface UserRole {
  ...
  useEffect(() => {
    ...
    socket.on('roleAssignment', (role: string) => { 
      setUserRole(role);
    });// 역할 할당을 처리하는 부분 추가
    ...
    <div className={`${styles.colMd8} ${styles.colMdOffset2} ${styles.bootstrap} ${styles.snippets} ${styles.bootdeys} ${styles.inbox_msg}`}>
      <div className={`${styles.widgetContainer} ${styles.scrollable} ${styles.list} ${styles.rollodex}`}>
        ...
        <h2>My role</h2>
          ...
          {userRole.role == 'Mafia' && <><Image src={mafia_reverse} alt="role" width={145} height={160}></Image>Mafia</>}
          {userRole.role == 'Police' && <><Image src={police_reverse} alt="role" width={145} height={160}></Image>Police</>}
          {userRole.role == 'Doctor' && <><Image src={doctor_reverse} alt="role" width={145} height={160}></Image>Doctor</>}
          {userRole.role == 'Citizen' && <><Image src={citizen} alt="role" width={145} height={160}></Image>Citizen</>}
          </div>
